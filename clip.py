...
class YouTubeShortsAutomation:
    def __init__(self):
        ...
        Path("processed_clips").mkdir(exist_ok=True)
        
        # Track processed clips
        self.processed_clips_file = "processed_clips/processed.json"
        self.processed_clips = self._load_processed_clips()
    ...
    def _load_processed_clips(self):
        """Load the set of already processed clip IDs"""
        try:
            if os.path.exists(self.processed_clips_file):
                with open(self.processed_clips_file, 'r') as f:
                    return set(json.load(f))
        except Exception as e:
            logger.error(f"Error loading processed clips: {e}")
        return set()

    def _save_processed_clips(self):
        """Save the set of processed clip IDs"""
        try:
            with open(self.processed_clips_file, 'w') as f:
                json.dump(list(self.processed_clips), f)
        except Exception as e:
            logger.error(f"Error saving processed clips: {e}")
    ...
    def process_clips(self):
        ...
        new_clips = [clip for clip in clips if clip['id'] not in self.processed_clips]
        
        if not new_clips:
            logger.info("No new clips to process")
            return 0
            
        processed_count = 0
        max_uploads = 3  # Limit uploads per run to avoid quota issues
        
        for clip in new_clips[:max_uploads * 2]:  # Process more clips to account for failures
            ...
            try:
                ...
                if self.upload_to_youtube_shorts(final_path, title, description, tags):
                    processed_count += 1
                    self.processed_clips.add(clip_id)  # Mark as processed
                    self._save_processed_clips()  # Save the updated list
